...
const REALITY_EDIT =
  /добав(ь|те) (найденн|эту балку|балку|колонн)|как на фото|перенес(и|ти) шахту|фактическ|as-built|подтверд|на этом фото|венткороб|две стойк|постав(ь|ьте) .*фото/i;
const REALITY_QUERY =
  /фото|видео|съёмк|съемк|объект(е|а)? реал|сопостав|маркер|контролн|reality|as-built|as-designed|вот фото/i;
const PROJECT_EDIT =
  /постав(ь|ьте)|увелич|уменьш|передвинь|перенес(и|ти)|добав(ь|те) (второй )?вентилятор|разверн|оптимизир|достич|довест|измени|сделай проём|проем/i;
const PROJECT_QUERY =
